...
db.getCollection('restaurants').find({cuisine: "Italian"}, {name: true, _id: false})

// 2. Provide a query showing how many Bakeries have a name that start with "M".

db.restaurants.count({ cuisine: "Bakery", name : /^M/ }, { name:true, _id:false })
db.getCollection('restaurants').find({ cuisine: "Bakery", name : /^M/ }, { name:true, _id:false })
...
db.getCollection('restaurants').find({ cuisine : /Ice/ }, { "address.zipcode": true, cuisine: true })
...
db.getCollection('restaurants').find({ cuisine: "Chinese" }, { "address.street": true, "address.building": true,  _id: false }).sort({ "address.zipcode": 1 })

// 5. Show only the American restaurants in Manhattan.

db.restaurants.find({ cuisine: "American", borough: "Manhattan" })
db.getCollection('restaurants').find({ cuisine: "American", borough: "Manhattan" })
...
db.getCollection('restaurants').find({ grades: { $size: 4 } })
...
db.getCollection('restaurants').find({ "address.street": "Broadway" }, { name:  true, grades: { $slice: 2 } })
...
db.restaurants.find({ "borough": "Brooklyn" }).skip( 20 ).sort({ name: 1 }).limit( 10 )
db.getCollection('restaurants').find({ "borough": "Brooklyn" }).skip( 20 ).sort({ name: 1 }).limit( 10 )
...
db.getCollection('restaurants').find({$or: [{cuisine: "Italian"}, {cuisine: "Pizza"}] }).sort({ name: -1 })
